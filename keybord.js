export default class Keyboard {
  constructor(layout) {
    ...
    Object.keys(layout).forEach((key) => {
      ...
      if (key === 'CapsLock') {
        this.button.classList.add('keyboard__btn_caps');
        this.button.addEventListener('mousedown', this.capsLock.bind(this));
      }
  ...
  capsLock() {
    ...
    this.capsKeys = { ...this.layout };
    Object.keys(this.capsKeys).forEach((key) => {
      if (this.capsKeys[key].length === 1)
        this.capsKeys[key] = this.capsKeys[key].toUpperCase();
    });
    if (this.capsOn) {
      Object.keys(this.capsKeys).forEach((key) => {
        document.getElementById(key).textContent = this.capsKeys[key];
      });
    } else {
      Object.keys(this.layout).forEach((key) => {
        document.getElementById(key).textContent = this.layout[key];
    ...
    document
      ...
      .classList.toggle('keyboard__btn_caps-on');
  }
  ...
  activeAdd() {
    ...
    const position = teaxArea.selectionStart;
